...
# First replace StatPill definition
...
# Now replace the calls in the main menu
# The main menu calls are right before:
#             Box(modifier = Modifier.weight(1f).fillMaxWidth()) {
#                 when (screen) {
#                     Screen.HOME -> {

import re
pattern = r'StatPill\(icon = Icons\.Default\.Favorite, tint = Color\(0xFFEF4444\), text = "\$\{appState\.lives\}", label = timerText\)\s*StatPill\(icon = Icons\.Default\.Star, tint = Color\(0xFFFBBF24\), text = "\$\{appState\.coins\}", label = ""\)\s*\}\s*Box\(modifier = Modifier\.weight\(1f\)\.fillMaxWidth\(\)\) \{'

replacement = '''StatPill(icon = Icons.Default.Favorite, tint = Color(0xFFEF4444), text = "${appState.lives}", label = timerText, isLarge = true)
